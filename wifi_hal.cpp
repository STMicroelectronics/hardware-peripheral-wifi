...
static wifi_error wifi_get_wake_reason_stats(wifi_interface_handle iface, WLAN_DRIVER_WAKE_REASON_CNT *wifi_wake_reason_cnt);
...
/*initialize function pointer table with HHAL API*/
wifi_error init_wifi_vendor_hal_func_table(wifi_hal_fn *fn)
{
    ...
    fn->wifi_get_supported_feature_set = wifi_get_supported_feature_set; //IWifiChip => getCapabilities
    ...
    fn->wifi_get_valid_channels = wifi_get_valid_channels; //IWifiApIface GetValidFrequenciesForBand
    ...
    fn->wifi_start_logging = wifi_start_logging; //IWifiChip StartLoggingToDebugRingBuffer
    ...
    fn->wifi_get_ring_buffers_status = wifi_get_ring_buffers_status; //IWifiChip GetDebugRingBuffersStatus
    fn->wifi_get_logger_supported_feature_set = wifi_get_logger_supported_feature_set;
    fn->wifi_get_ring_data = wifi_get_ring_data; //IWifiChip forceDumpToDebugRingBuffer
    ...
    fn->wifi_get_packet_filter_capabilities = wifi_get_packet_filter_capabilities; //IWifiChip GetApfPacketFilterCapabilities
    fn->wifi_set_packet_filter = wifi_set_packet_filter;
    fn->wifi_get_wake_reason_stats = wifi_get_wake_reason_stats; //IWifiChip getDebugHostWakeReasonStats
    return WIFI_SUCCESS;
}

/* API to collect wifi wake up reason s */
static wifi_error wifi_get_wake_reason_stats(wifi_interface_handle /*iface*/, WLAN_DRIVER_WAKE_REASON_CNT *wifi_wake_reason_cnt)
{
    ...
    cmd->releaseRef();
    return result;*/
    wifi_wake_reason_cnt->cmd_event_wake_cnt_used = 0;
    wifi_wake_reason_cnt->driver_fw_local_wake_cnt_used = 0;
    return WIFI_SUCCESS;
}
...
void wifi_cleanup(wifi_handle handle, wifi_cleaned_up_handler handler)
{
    ...
    if (TEMP_FAILURE_RETRY(write(info->cleanup_socks[0], "Exit", 4)) < 1) {
        ...
        ALOGD("%s: Read after POLL returned %zd, error no = %d (%s)", __FUNCTION__,
               result, errno, strerror(errno));
        if (strncmp(buf, "Done", 4) == 0) {
            ALOGD("Event processing terminated");
        } else {
            ALOGD("Rx'ed %s", buf);
        }
    }
...
{
    /* Possible values for feature_set
    ...
    WIFI_FEATURE_LINK_LAYER_STATS   0x10000     // Link layer stats collection
    ...
    WIFI_FEATURE_SET_TX_POWER_LIMIT 0x4000000   // Support Tx Power Limit setting
    WIFI_FEATURE_USE_BODY_HEAD_SAR  0x8000000
    */
